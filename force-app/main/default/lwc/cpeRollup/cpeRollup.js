...
export default class RollupConfig extends CustomPropertyEditor {
	...
	generateInputs() {
		const rollupInput = new Input("Rollup Metadata Record(s)", "rollups", DataTypes.REFERENCE)
			...
			?.addOptionsFromFlow(
				this.getFlowResources()?.filter(
					(resource) => resource?.dataType === "SObject" && resource.objectType === ROLLUP_CMDT?.objectApiName
				)
			);
		...
		const timingInput = new Input("Timing", "timing", DataTypes.STRING)
			?.addOptions([
				new ComboboxOption("Asynchronous", "ASYNCHRONOUS"),
				new ComboboxOption("Synchronous", "SYNCHRONOUS")
			])
